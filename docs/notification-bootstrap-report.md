...
Generated: 2026-07-22T13:57:13.901Z
Bootstrap script version: 1.0.0
Database: zutsav_nitesh @ localhost
...
## Created
- `OTP_VERIFICATION` / user (`6a60cc39f00c22b6f6cd4c04`) → `whatsapp_verification`
- `OTP_VERIFICATION` / pandit (`6a60cc39f00c22b6f6cd4c13`) → `whatsapp_verification`
- `SERVICE_COMPLETION_OTP` / user (`6a60cc39f00c22b6f6cd4c17`) → `whatsapp_verification`
- `DELIVERY_OTP_SENT` / user (`6a60cc39f00c22b6f6cd4c1a`) → `whatsapp_verification`
- `PASSWORD_RESET_WHATSAPP_OTP` / user (`6a60cc39f00c22b6f6cd4c1d`) → `whatsapp_verification`
- `PASSWORD_RESET_WHATSAPP_OTP` / pandit (`6a60cc39f00c22b6f6cd4c21`) → `whatsapp_verification`
- `USER_REGISTERED` / user (`6a60cc39f00c22b6f6cd4c24`) → `new_user_registered`
- `PAYMENT_SUCCESS` / user (`6a60cc39f00c22b6f6cd4c27`) → `payment_success`
- `PARTIAL_PAYMENT_RECEIVED` / user (`6a60cc39f00c22b6f6cd4c2a`) → `partial_payment_received`
- `FINAL_PAYMENT_RECEIVED` / user (`6a60cc39f00c22b6f6cd4c2d`) → `final_payment_received`
- `PAYMENT_FAILED` / user (`6a60cc39f00c22b6f6cd4c30`) → `payment_failed`
- `BOOKING_CONFIRMED` / user (`6a60cc39f00c22b6f6cd4c33`) → `booking_confirmed`
- `BOOKING_CANCELLED` / user (`6a60cc39f00c22b6f6cd4c36`) → `booking_cancelled`
- `BOOKING_REFUNDED` / user (`6a60cc39f00c22b6f6cd4c39`) → `booking_refunded`
- `SERVICE_REMINDER_24H` / user (`6a60cc39f00c22b6f6cd4c3c`) → `service_reminder_24h`
- `SERVICE_REMINDER_1H` / user (`6a60cc39f00c22b6f6cd4c3f`) → `service_reminder_1h`
- `SERVICE_COMPLETED` / user (`6a60cc39f00c22b6f6cd4c42`) → `service_completed`
- `INVOICE_GENERATED` / user (`6a60cc39f00c22b6f6cd4c45`) → `invoice_generated`
- `FEEDBACK_REQUEST` / user (`6a60cc39f00c22b6f6cd4c48`) → `feedback_request`
- `KIT_SHIPPED` / user (`6a60cc39f00c22b6f6cd4c4b`) → `kit_shipped`
- `KIT_DELIVERED` / user (`6a60cc39f00c22b6f6cd4c4e`) → `kit_delivered`
- `ORDER_CONFIRMED` / user (`6a60cc39f00c22b6f6cd4c51`) → `order_confirmed`
- `ORDER_PACKED` / user (`6a60cc39f00c22b6f6cd4c54`) → `order_packed`
- `ORDER_SHIPPED` / user (`6a60cc39f00c22b6f6cd4c57`) → `order_shipped`
- `ORDER_OUT_FOR_DELIVERY` / user (`6a60cc39f00c22b6f6cd4c5a`) → `order_out_for_delivery`
- `ORDER_DELIVERED` / user (`6a60cc39f00c22b6f6cd4c5d`) → `order_delivered`
- `ORDER_CANCELLED` / user (`6a60cc39f00c22b6f6cd4c60`) → `order_cancelled`
- `ORDER_REFUNDED` / user (`6a60cc39f00c22b6f6cd4c63`) → `order_refunded`
- `PANDIT_POOJA_REJECTED` / pandit (`6a60cc39f00c22b6f6cd4c66`) → `pandit_pooja_rejected`
- `PANDIT_REGISTERED` / pandit (`6a60cc39f00c22b6f6cd4c69`) → `pandit_registered`
- `PANDIT_POOJA_APPROVED` / pandit (`6a60cc39f00c22b6f6cd4c6c`) → `pandit_pooja_approved`
- `PANDIT_APPROVED` / pandit (`6a60cc39f00c22b6f6cd4c6f`) → `pandit_approved`
- `KYC_APPROVED` / pandit (`6a60cc39f00c22b6f6cd4c72`) → `kyc_approved`
- `KYC_REJECTED` / pandit (`6a60cc39f00c22b6f6cd4c75`) → `kyc_rejected`
- `KYC_REUPLOAD_REQUIRED` / pandit (`6a60cc39f00c22b6f6cd4c78`) → `kyc_reupload_required`
- `ACCOUNT_RESTORED` / user (`6a60cc39f00c22b6f6cd4c7b`) → `account_restored`
- `ACCOUNT_DELETED` / user (`6a60cc39f00c22b6f6cd4c7e`) → `account_deleted`
- `ACCOUNT_DELETION_CANCELLED` / user (`6a60cc39f00c22b6f6cd4c81`) → `account_deletion_cancelled`
- `ACCOUNT_DELETION_REQUESTED` / user (`6a60cc39f00c22b6f6cd4c84`) → `account_deletion_requested`
- `REFERRAL_BOOKING_CREATED` / referral_pandit (`6a60cc39f00c22b6f6cd4c87`) → `referral_booking_created`
- `PAYOUT_RELEASED` / pandit (`6a60cc39f00c22b6f6cd4c8a`) → `payout_released`
