...
const { normalizePanditPayload } = require('../../notification-engine/variables/PayloadNormalizer');
...
exports.createReferral = async (req, res, next) => {
  try {
    const { userMobile, userEmail, poojaId, preferredDate, performByReferringPandit, referralReason } = req.body;
    ...
    // performByReferringPandit defaults to true; explicit false requires a reason
    const willPerform = performByReferringPandit !== false;
    if (!willPerform) {
      const reason = (referralReason || '').trim();
      if (!reason) {
        return res.status(400).json({
          success: false,
          message: 'A reason is required when you will not personally perform the pooja.',
        });
      }
    }
    ...
    const referral = await Referral.create({
      ...
      performByReferringPandit:  willPerform,
      referralReason:            willPerform ? '' : (referralReason || '').trim(),
      expiresAt,
...
exports.getMyReferrals = async (req, res, next) => {
  try {
    ...
    // EXPIRED is a virtual filter — referrals that are past expiry and not yet booked
    if (status === 'EXPIRED') {
      query.expiresAt = { $lt: new Date() };
      query.status    = { $in: ['CREATED', 'SENT', 'OPENED'] };
    } else if (status) {
      query.status = status;
    }
...
exports.submitRemark = async (req, res, next) => {
  try {
    ...
    NotificationEngine.emit(
      'REFERRAL_REMARK_SUBMITTED',
      normalizePanditPayload({ pandit, reason: trimmed })
    ).catch(() => {});
