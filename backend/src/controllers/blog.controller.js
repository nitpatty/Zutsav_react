...
exports.createBlog = async (req, res, next) => {
  try {
    ...
    if (req.body.action === 'publish' || req.body.action === 'submit') {
      if (isAdminRole(user.role) && settings.blogAdminRequireApproval !== true) {
        status = 'published';
...
exports.updateBlog = async (req, res, next) => {
  try {
    ...
    // Submit a draft, or re-submit a rejected blog, for review
    if (req.body.action === 'submit' && ['draft', 'rejected'].includes(blog.status)) {
      updates.status         = 'pending_review';
      updates.rejectionReason = '';
    }
    if (req.body.action === 'publish' && isAdminRole(req.user.role)) {
...
exports.shareBlog = async (req, res, next) => {
  try {
    ...
    await Blog.findByIdAndUpdate(blog._id, { $inc: { shares: 1 } });
    const baseUrl = urls.clientUrl;
    res.json({ success: true, shares: blog.shares + 1, shareUrl: `${baseUrl}/blog/${blog.slug}` });
  } catch (err) { next(err); }
};
...
exports.deleteComment = async (req, res, next) => {
  try {
    ...
    if (String(comment.authorId) !== String(req.user._id) && !isAdminRole(req.user.role)) {
      return res.status(403).json({ success: false, message: 'Not authorized' });
    }
...
exports.checkPermission = async (req, res, next) => {
  try {
    ...
    const canPublish = (
      (isAdminRole(role) && settings?.blogAdminPublish  !== false) ||
