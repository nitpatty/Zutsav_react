...
// Aliases for the company's registered state (default fallback if the
// System Configuration Center's companyState field isn't set). Used for
// IGST vs CGST+SGST determination.
const DEFAULT_COMPANY_STATE_KEYS = ['uttarpradesh', 'up', 'uttar pradesh'];

async function _companyStateKeys() {
  const state = await settingsService.get('companyState', companyConfig.state);
  const normalized = String(state || '').toLowerCase().replace(/\s+/g, '');
  return normalized ? [normalized] : DEFAULT_COMPANY_STATE_KEYS;
}
...
// ── GST split helper ──────────────────────────────────────────────────────────
function buildGstBreakdown(totalGST, customerState = '', companyStateKeys = DEFAULT_COMPANY_STATE_KEYS) {
  const cs = customerState.toLowerCase().replace(/\s+/g, '');
  const isInterstate = !companyStateKeys.includes(cs);
  return isInterstate
...
async function generateInvoiceForPayment(booking, ledger, poojaName = '') {
  try {
    ...
    const companyStateKeys = await _companyStateKeys();
    const gstBreakdown    = buildGstBreakdown(totalGST, booking.userDetails?.state || '', companyStateKeys);
