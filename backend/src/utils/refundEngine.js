...
const { roundToPaise } = require('./financeUtils');
...
function calculateRefundBreakdown(booking) {
  const amountPaid  = roundToPaise(booking.amountPaid  || 0);
  const platformFee = roundToPaise(booking.platformFee || 0);
  const platformGST = roundToPaise(booking.platformGST || 0);
