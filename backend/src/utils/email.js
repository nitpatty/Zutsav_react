...
const { company: companyConfig, urls: urlsConfig } = require('../config');

/**
 * Company/support info shown in emails — DB value (System Configuration
 * Center) wins, falls back to the env-derived config/company.config.js
 * defaults. Previously each caller hardcoded these independently.
 */
async function _companyInfo() {
...
const sendInvoiceEmail = async (booking, poojaName) => {
  if (!booking.userDetails?.email) return;
  const co = await _companyInfo();
  ...
  <div style="font-family:'Segoe UI',sans-serif;max-width:600px;margin:0 auto;background:#f8f9fa;padding:0 0 32px">
    ...
    <div style="background:white;padding:28px 32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
      ...
      <div style="text-align:center;margin-bottom:20px">
        <a href="${co.websiteUrl}/my-bookings" style="display:inline-block;padding:12px 28px;background:#1B1F3B;color:white;font-weight:700;font-size:14px;border-radius:10px;text-decoration:none">
  ...
  <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:620px;margin:0 auto;background:#f8f9fa;padding:0 0 32px">
    ...
    <div style="background:white;padding:16px 32px 24px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
      ...
      <p style="color:#9ca3af;font-size:12px;text-align:center;margin:0;line-height:1.7">
        ...
        For support: ${co.email} &nbsp;|&nbsp; ${co.phone}
      </p>
    </div>
  </div>`;

  return sendEmail(email, `Tax Invoice — Order #${order.orderNumber} | ${co.name}`, html, {
...
const sendDeliveryOTPEmail = async (order, user, otp) => {
  ...
  <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:560px;margin:0 auto;background:#f8f9ff;padding:0 0 24px">
    ...
    <div style="background:white;padding:28px 32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
      ...
      <p style="color:#6b7280;font-size:13px;margin:0">If you did not expect this delivery, contact us immediately at ${(await _companyInfo()).email}</p>
    </div>
  </div>`;
...
async function _orderEmailWrapper(title, bodyHtml) {
  const co = await _companyInfo();
  return `
  <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:560px;margin:0 auto;background:#f8f9ff">
    <div style="background:#1B1F3B;padding:24px 32px;border-radius:12px 12px 0 0;text-align:center">
      <h1 style="color:#D4AF37;font-family:Georgia,serif;font-size:22px;margin:0">🪔 ${co.name}</h1>
    ...
    <div style="background:white;padding:28px 32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
      ...
      <p style="color:#6b7280;font-size:12px;margin-top:24px;border-top:1px solid #e5e7eb;padding-top:12px">
        You can track your order anytime from <a href="${co.websiteUrl}/my-orders" style="color:#1B1F3B;font-weight:600">My Orders</a>.
      </p>
    </div>
  </div>`;
}
...
const sendOrderShippedEmail = async (order, shipment) => {
  ...
  const body = `
    ...
    <p style="color:#b91c1c">🙏 Team Zutsav</p>`;
  const html = await _orderEmailWrapper('Your Order is on the Way! 🚚', body);
  return sendEmail(userEmail, `Your Order #${order.orderNumber} Has Been Shipped!`, html, {
...
const sendOrderStatusEmail = async (order, status) => {
  ...
  const html = await _orderEmailWrapper(meta.heading, bodyHtml);
  return sendEmail(
    userEmail,
