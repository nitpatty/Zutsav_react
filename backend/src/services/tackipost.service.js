...
const { tekipost } = require('../config/integrations.config');

const BASE_URL = tekipost.baseUrl;
...
function _checkEnv() {
  ...
  required.forEach((key) => {
    if (!process.env[key]) {
      console.warn(`[TekiPost] WARNING: env var ${key} is not set — set it in .env before creating shipments.`);
    }
  });
}
...
async function getToken() {
  ...
  const email    = process.env.TEKIPOST_EMAIL;
  const password = process.env.TEKIPOST_PASSWORD;

  if (!email || !password) {
    throw new Error('TekiPost is not configured: set TEKIPOST_EMAIL and TEKIPOST_PASSWORD in .env or Admin → System Settings.');
  }
