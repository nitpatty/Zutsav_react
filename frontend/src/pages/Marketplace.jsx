            ...
            <div className="flex items-end justify-between gap-2">
              <div>
                    ...
                    <div className="absolute left-0 top-full mt-1 z-30 bg-white border border-gray-200 rounded-xl shadow-lg min-w-[150px] overflow-hidden">
                      {p.variants.filter((v) => v.isActive !== false).map((v) => (
                        <button key={v.variantId}
                          ...
                          <span className="font-bold">₹{(v.salePrice || v.price).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</span>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div className="flex items-baseline gap-1.5">
                  <span className="font-display text-lg font-bold text-saffron-600">₹{displayPrice?.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</span>
                  {slashedPrice && <span className="text-xs text-gray-400 line-through font-sans">₹{slashedPrice.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</span>}
                </div>
              </div>
