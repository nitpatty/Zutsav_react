      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        {pooja && (
          <div className="bg-white rounded-2xl shadow p-5 flex gap-4 items-start">
            {pooja.image && (
              <img src={getImageUrl(pooja.image)} alt={pooja.name} className="w-16 h-16 rounded-xl object-cover shrink-0" />
            )}
            <div className="flex-1 min-w-0">
              ...
              {(pooja.salePrice || pooja.price) && (
                <p className="text-sm font-bold text-green-700 mt-1">
                  ₹{(pooja.salePrice || pooja.price).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
                </p>
              )}
            </div>
          </div>
