...
function PoojasTab({ pandit, reload }) {
  ...
  const [reqForm, setReqForm] = useState({ poojaName: '', description: '', estimatedDuration: '', estimatedDurationUnit: 'hours', categoryId: '', expectedPrice: '' });
  const [categories, setCategories] = useState([]);
  ...
  const submitRequest = async (e) => {
    e.preventDefault();
    if (!reqForm.poojaName || !reqForm.categoryId) { toast.error('Pooja name and category are required'); return; }
    const price = Number(reqForm.expectedPrice);
    if (!Number.isFinite(price) || price <= 0) { toast.error('Expected Price must be a number greater than zero'); return; }
    if (!reqForm.estimatedDuration) { toast.error('Estimated duration is required'); return; }
    setSubmitting(true);
    try {
      const fd = new FormData();
      Object.entries(reqForm).forEach(([k, v]) => { if (v) fd.append(k, v); });
      const { data } = await API.post('/pandit/pooja-requests', fd);
      setMyRequests((p) => [data.request, ...p]);
      setReqForm({ poojaName: '', description: '', estimatedDuration: '', estimatedDurationUnit: 'hours', categoryId: '', expectedPrice: '' });
      setShowRequest(false);
      toast.success('Pooja request submitted! Pending admin approval.');
    } catch (err) { toast.error(err.response?.data?.message || 'Failed to submit request'); } finally { setSubmitting(false); }
  };
          ...
          <form onSubmit={submitRequest} className="space-y-3 border-t border-gray-100 pt-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input className="input text-sm" value={reqForm.poojaName}
                  onChange={(e) => setReqForm((p) => ({ ...p, poojaName: e.target.value }))} placeholder="e.g. Mahamrityunjaya Jaap" />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  value={reqForm.expectedPrice}
                  onChange={(e) => setReqForm((p) => ({ ...p, expectedPrice: e.target.value }))}
                />
                <p className="text-xs text-gray-400 mt-1">What you believe is a fair service charge.</p>
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="e.g. 2"
                    value={reqForm.estimatedDuration}
                    onChange={(e) => setReqForm((p) => ({ ...p, estimatedDuration: e.target.value }))}
                  />
                  <select
                    className="input text-sm flex-1"
                    value={reqForm.estimatedDurationUnit}
                    onChange={(e) => setReqForm((p) => ({ ...p, estimatedDurationUnit: e.target.value }))}
                  >
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 flex gap-2 text-xs text-amber-700">
              ...
              Your request will be reviewed by admin. It will appear for bookings only after approval, at the admin-approved price.
            </div>
          </form>
        )}
        {myRequests.length > 0 && (
          <div className="space-y-2 border-t border-gray-100 pt-3">
            <div className="flex items-center justify-between">
              ...
              <Link to="/pandit/requested-poojas" className="text-xs text-saffron-600 hover:underline">View all</Link>
            </div>
            {myRequests.slice(0, 5).map((r) => (
              <div key={r._id} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-2.5">
                <div>
                  <p className="text-sm font-medium text-gray-700">{r.poojaName}</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    Expected {fmtINR(r.expectedPrice)}
                    {r.status === 'approved' && <> · Approved {fmtINR(r.adminApprovedPrice)}</>}
                  </p>
                  {r.status === 'rejected' && r.rejectionReason && <p className="text-xs text-red-500 mt-0.5">{r.rejectionReason}</p>}
                </div>
                <span className={`text-xs px-2.5 py-0.5 rounded-full font-medium shrink-0 ${STATUS_BADGE[r.status] || 'bg-gray-100 text-gray-500'}`}>
                  {r.status}
                </span>
              </div>
            ))}
