            ...
            {poojas.map((p) => (
              ...
              >
                <div className="h-48 bg-saffron-100 overflow-hidden relative">
                  {p.image
                    ? <img src={getImageUrl(p.image)} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-baseline gap-2">
                      ...
                      {p.mrp && p.mrp > (p.salePrice || p.price) && (
                        <span className="text-sm text-gray-400 line-through">₹{p.mrp.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</span>
                      )}
                    </div>
                    <span
