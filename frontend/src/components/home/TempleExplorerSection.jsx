    ...
    <section className="section-pad bg-white">
      <div ref={templeRef} className="container-pad">
        <div className="text-center mb-14">
          <EyebrowTag>{t('home.sacredPlaces', 'Sacred Places')}</EyebrowTag>
          <h2 className="section-title">{t('home.templeExplorer', 'Temple Explorer')}</h2>
          <p className="section-subtitle mx-auto text-center">
            {t('home.templeExplorerSubtitle', "Explore India's most revered temples and plan your sacred journey")}
          </p>
        </div>
                ...
                return (
                  <div key={temple._id}
                    ...
                    <div className={`h-40 bg-gradient-to-br ${gFrom} ${gTo} flex items-center justify-center relative overflow-hidden`}>
                      {image ? (
                        <img src={IMG(image)} alt={temple.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" loading="lazy" />
                      ) : (
                        <>
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-2">
                        <h3 className="font-display font-bold text-xl text-gray-900" style={{ letterSpacing: '-0.01em' }}>{temple.name}</h3>
                      </div>
        ...
        <div className="text-center mt-12">
          <Link to="/temples" className="btn-outline inline-flex items-center gap-2">
            {t('home.viewAllTemples', 'View All Temples')} <ArrowRight size={16} />
          </Link>
        </div>
      </div>
    </section>
